...
/**
 * Initializes the renderer with labels fetched from the backend config API
 */
export function initRendererConfig(config) {
...
export function renderGearCard(gear, containerEl) {
  ...
  const setLabel = setLabels[gear.set] || gear.set.replace('Set', '');
  ...
  // Read pre-calculated scores from Go backend API
  const gs = gear.score ? gear.score.gs : 0;
  const es = gear.score ? gear.score.es : 0;
